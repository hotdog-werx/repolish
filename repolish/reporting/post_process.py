...
marker and a details link to the text report. Sessions that ran nothing
contribute no group, so the tree is silent when post-process is unused.
"""
...
def _group_label(session: 'ResolvedSession') -> str:
    """Name the post-process group after the session's directory role."""
...
def _command_text(outcome: CommandOutcome) -> Text:
    """One command row: marker, the raw argv as written, outcome, details link."""
    text = Text()
...
def _run_children(
    ...
    for outcome in run.outcomes:
        label = _command_text(outcome)
        details_link(label, report_path)
        children.append(SummaryNode(label=label))
    return children
...
def post_process_nodes(
    ...
    The session's own run hangs its command rows directly under the group;
    a promoted-files run (root monorepo pass) nests as its own labeled
    subgroup with a details link of its own.
    """
    ...
    for session in sessions:
        ...
        group = SummaryNode(
            label=_group_suffix_text(_group_label(session), all_outcomes),
        )
        for (label, run), report_path in zip(
            ...
            if label == 'session':
                group.children.extend(_run_children(run, report_path))
            else:
                ...
                group.children.append(
                    SummaryNode(
                        label=sub_label,
                        children=_run_children(run, report_path),
                    ),
                )
        nodes.append(group)
    return nodes
