"""The input contracts: what each finished run hands to summaries.

Three surfaces, three contracts, one shape each:
...
Both protocols are structural, so the apply pipeline's `ResolvedSession`
