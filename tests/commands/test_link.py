...
def test_link_uses_cache_when_already_linked(tmp_path: Path) -> None:
    """Second run of `repolish link --no-force` uses cache and shows (cached) message."""
    ...
    # Second run with --no-force - should use cache and show (cached) message
    output2 = _run_link(['--no-force'], tmp_path)
    assert '(cached)' in output2


def test_link_force_ignores_cache(tmp_path: Path) -> None:
    """`repolish link` (force=True by default) re-links even when cache is valid."""
    # Create a minimal static provider
    provider_dir = tmp_path / 'provider'
    provider_dir.mkdir()
    (provider_dir / 'repolish.py').write_text(
...
def test_link_no_force_uses_cache(tmp_path: Path) -> None:
    """`repolish link --no-force` uses cache when already linked."""
    # Create a minimal static provider
    provider_dir = tmp_path / 'provider'
    provider_dir.mkdir()
    (provider_dir / 'repolish.py').write_text(
