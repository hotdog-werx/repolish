...
_LANE_PROVIDER = """\
    ...
    class P(Provider[Ctx, BaseInputs]):
        ...
        def create_file_mappings(self, context):
            return {'README.md': 'README.md.jinja'}

        def create_fast_lanes(self, repolish):
            return {
                'actions': FastLaneSpec(
                    file_mappings={
                        'action.yaml': TemplateMapping(
                            '_repolish.action.yaml.jinja',
                            extra_context={'alias': repolish.provider.alias},
                        ),
                    },
                ),
            }
...
def test_apply_fast_lane_writes_only_lane_outputs(tmp_path: Path) -> None:
    provider_cls = _make_provider_pkg(
        tmp_path,
        _LANE_PROVIDER,
        templates={
            'README.md.jinja': 'full apply only\n',
            '_repolish.action.yaml.jinja': 'generated by {{ alias }}\n',
        },
    )
    project = tmp_path / 'project'
    project.mkdir()

    result = apply_fast_lane(provider_cls, project, 'actions', alias='demo')

    assert result.exit_code == 0
    assert result.apply_result['action.yaml'] == 'written'
    lane_files = include_paths(result.managed_files(), exact={'action.yaml'})
    assert lane_files == {'action.yaml': 'generated by demo\n'}


def test_apply_fast_lane_check_only_reports_drift_then_converges(
    tmp_path: Path,
) -> None:
    provider_cls = _make_provider_pkg(
        tmp_path,
        ...
        templates={
            'README.md.jinja': 'full apply only\n',
            '_repolish.action.yaml.jinja': 'generated by test-provider\n',
        },
    )
    project = tmp_path / 'project'
    project.mkdir()

    drift = apply_fast_lane(provider_cls, project, 'actions', check_only=True)
    assert drift.exit_code == 2
    assert not (project / 'action.yaml').exists()

    applied = apply_fast_lane(provider_cls, project, 'actions')
    assert applied.exit_code == 0

    clean = apply_fast_lane(provider_cls, project, 'actions', check_only=True)
