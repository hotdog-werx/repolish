...
### Workflow: bootstrap with a command, maintain with a lane
...
This also avoids the older workflow where users had to create an empty file,
run repolish to populate it, and then run repolish again to converge on the
final state. Provider commands make that bootstrap step immediate, and fast
lanes keep the ongoing maintenance pass cheap.
