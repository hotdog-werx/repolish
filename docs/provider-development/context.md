...
`create_file_validators()` lets a provider enforce project-level correctness
checks against files in the project or render tree. This is a good fit for rules
like "`config.toml` needs a generated header", "the file must end with a
newline", or "the selected Python version matches the project settings".

A validator does not need a matching `create_file_mappings()` entry. Providers
may validate an existing project file that is already present in the repo and is
not rendered by repolish at all. In that case the file is still shown in the
summary, but the status is explicit: there is no file in the render stage for
it, so the summary shows a non-write marker instead of a green written marker.

The registry is keyed by destination path and then by validator name. Each
validator can be registered either as a bare callable or as a typed spec with
options:
...
The public contract is intentionally typed:

- `FileValidatorOptions(enabled=True)` turns a validator on or off.
- `FileValidatorSpec(fn=..., options=...)` is the structured form.
- `ValidatorFn` receives the provider context and the resolved file path.
- `ValidationResult.status` is a `ValidationStatus` enum, not a loose boolean.
- `ValidationStatus.PASS`, `WARNING`, and `ERROR` let the UI and strict mode
  distinguish success, warnings, and failures clearly.

The repo config is the final authority: provider defaults are merged first, then
project overrides are applied. That means a project can enable a default-off
validator, disable a default-on validator, or keep a provider-specified
validator visible but inactive in the summary without removing it from the
registry.

Validator-only registrations are also additive. If a provider defines validation
against `README.md` without creating a file mapping for it, that check still
runs against the actual repo file and is reported separately from "written"
files. The summary uses a distinct marker to communicate this state:
...
This means the file is still meaningful to the provider, but it was not staged
or materialized by repolish during this run.

### Disabling validators in `repolish.yaml`

Validator overrides follow the same pattern as file mapping overrides. The
project may disable a validator without deleting the provider declaration, which
keeps the disabled state visible in the `repolish apply` summary instead of
silently erasing it:
...
When this is set, `lint` remains in the registry but is treated as `disabled`
...
### Multiple providers can contribute to the same file

Validator registries are additive. Providers do not fight for the same file
path; they can each contribute their own validators to the same destination and
the results are shown together in the apply summary:
...
This means a provider can enforce a generated-file header while another provider
checks trailing whitespace or schema validity, without one silently replacing
the other.

### Apply summary output

When `repolish apply` runs, validators are rendered beneath the affected file as
separate lines so each result is easy to read and debug:
...
The same output model is used for warnings and failures:

```text
config.toml
  validators:
    - ⚠ lint: config is missing a recommended key
    - ✗ schema: missing version key
```

This keeps the signal explicit:

- green check mark: validator passed
- red cross: validator failed and reports why
- yellow warning: validator ran and produced a warning
- yellow cross: validator exists but is disabled by config
- hollow marker (`◌`): file has validation hooks but no file was staged for it
  in this apply run; the path is provider-owned, but not written

That makes it easy to understand both the provider's intent and the project's
local overrides without needing to read the provider source or documentation
just to know what one of the checks was doing.

### Monorepo mode and validator inheritance

Validator hooks follow the same mode-aware dispatch as the other provider hooks.
A provider can define `create_file_validators()` directly on the base provider,
and it is also valid to attach mode-specific handlers such as `root_mode`,
`member_mode`, and `standalone_mode` that implement their own validator logic.

The dispatch order is:

1. direct method on the provider, when overridden
2. matching mode handler for the current workspace mode
3. the base no-op default

In practice, this means you do not need to duplicate validator definitions for
all three modes unless a given mode needs different checks. The same validator
registry contract applies in every mode, and the current workspace mode decides
...
**Monorepo note:** `opt.all_providers` is scoped to the providers active in the
current run, not the entire workspace. In a monorepo root pass, that means the
root-level providers for that run; member providers are processed in separate
per-package passes and do not appear here. Likewise, `ModeHandler` dispatch is
chosen from the active workspace mode (`root`, `member`, or `standalone`), so a
validator or hook you see in one run may not exist in another. If you need data
that originates in a member package, prefer the `provide_inputs()` push pattern
from the member toward the root instead of reading a peer from `all_providers`.
